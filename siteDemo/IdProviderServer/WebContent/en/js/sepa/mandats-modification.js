...
Ext.onReady(function(){
    ...
    document.getElementById('html_mandats_modification_title').innerHTML = lang.html_mandats_modification_title;
    
    window.top.currentPage = "Modify-mandate";
    ...
    if (window.top.mandatToModify != -1) {
        ...
        var creationFieldSet = new Ext.form.FieldSet({
            id: 'creationFieldSet',
            title: lang.mandats_modification_form,
            autoWidth: true,
            autoHeight: true,
            animCollapse: false,
            collapsible: false,
        ...
        var mandatCreationForm = new Ext.form.FormPanel({
            ...
            buttons: [{
                text: lang.mandats_modification_button_cancel,
                handler: function(){
                    Ext.MessageBox.confirm(lang.mandats_modification_popup_cancelation_title, lang.mandats_modification_popup_cancelation_desc, function(btn, text){
                        if (btn == "yes") {
                            //Redirection
                ...
                text: lang.mandats_modification_button_create,
                handler: function(){
                    ...
                    if (found) {
                        Ext.MessageBox.show({
                            title: lang.mandats_modification_popup_impossible_title,
                            msg: lang.mandats_modification_popup_impossible_desc,
                        ...
                        periodField.getValue() == "")) {
                            Ext.MessageBox.show({
                                title: lang.mandats_modification_popup_validation_title,
                                msg: lang.mandats_modification_popup_validation_desc,
                                buttons: Ext.MessageBox.OK
                            });
                        }
                        else {
                            Ext.MessageBox.confirm(lang.mandats_modification_popup_confirmation_title, lang.mandats_modification_popup_confirmation_desc, function(btn, text){
                                if (btn == "yes") {
                                    //Creation
                                    modifyCurrentMandat(currentMandat);
                                    //Redirection
                                    if (window.top.callback_url != "") {
                                        var url = window.top.callback_url;
                                        window.top.callback_url = "";
...
function modifyCurrentMandat(currentMandat, status){
    var frequency = Ext.ComponentMgr.get('periodField').getValue();
    var creditor = Ext.ComponentMgr.get('creditorField').getValue();
    ...
    if (status == null || status == '') {
        status = currentMandat.status;
    }
    
    var uir = "";
    var underlyingContract = "";
    
    var id = currentMandat.id;
    
    //Cancel des paymentSchedule existant pour ce mandat
    var currentPaymentScheduleArray = paymentScheduleArray;
    for (var i = 0; i < currentPaymentScheduleArray.length; i++) {
        if (currentPaymentScheduleArray[i].mandatId == id) {
            paymentScheduleArray = removeFromArray(paymentScheduleArray, currentPaymentScheduleArray[i]);
        }
    }
    window.top.paymentScheduleArray = paymentScheduleArray;
    ...
    for (var i = 0; i < mandatArray.length; i++) {
        if (mandatArray[i].id == currentMandat.id) {
            mandatArray[i] = mandat;
        }
    }
}
